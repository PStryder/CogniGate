"""CogniGate's binding to the shared MetaGate bootstrap client.
...
CogniGate-specific thing: which primitive types map to which settings.
...
async def bootstrap_from_metagate(settings: Any) -> Any:
    """Resolve CogniGate's peer endpoints from MetaGate."""
    if _shared is None:
        logger.warning(
